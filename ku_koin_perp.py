...
INSTS = ["TONUSDTM", "SUIUSDTM", "APTUSDTM", "NEARUSDTM", "ATOMUSDTM", "AVAXUSDTM", "DOTUSDTM", "UNIUSDTM", "PEPEUSDTM",
             "RENDERUSDTM", "TRUMPUSDTM", "FILUSDTM", "JUPUSDTM", "PENGUUSDTM"]
def get_bullet():
    r = requests.post("https://api-futures.kucoin.com/api/v1/bullet-public").json()
    #print(r)
    data = r["data"]
    ...
    while True:
        try:
            ...
            async with websockets.connect(ws_url, ping_interval=None) as ws:
                ...
                print('Connected_kucoin_perp')

                for inst in INSTS:
                    await ws.send(json.dumps({
                        "id": str(int(time.time() * 1000)),
                        "type": "subscribe",
                        "topic": f"/contractMarket/level2Depth5:{inst}",
                        "response": True
                    }))
                ...
                async for msg in ws:
                    ...
                    instId = inform["topic"].split(":", 1)[1].replace("USDTM","-USDT")
                    data = inform["data"]
                    # print(data)
